...
game_sql="SELECT game_id from Post_collect_slots p, Games g WHERE p.game_id IS NOT NULL AND g.id = p.game_id AND (p.last_dumped IS NULL OR g.deadline_speed = 'Fast' OR (TIMESTAMPDIFF(MINUTE, p.last_dumped, NOW()) > 4) OR (TIME_TO_SEC(timediff(TIME(g.lynch_time), CURRENT_TIME())) BETWEEN -300 AND 1800));"
