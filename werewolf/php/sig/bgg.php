...
function send_geekmail($to, $subject, $message, $from="USERNAME", $password="PASSWORD" ) {

system("/var/www/html/php/send_geekmail.pl \"$from\" \"$password\" \"$to\" \"$subject\" \"$message\" > /dev/null &", $retval);

}

function reply_thread($thread_id, $body, $player="USERNAME", $password="PASSWORD" ) {
...
function reply_thread_quick($thread_id, $body, $player="USERNAME", $password="PASSWORD" ) {

system ("/var/www/html/php/post_thread.pl \"$player\" \"$password\" \"reply\" \"$thread_id\" \"$body\" > /dev/null &", $retval);

}

function edit_post($article_id, $body, $player="USERNAME", $password="PASSWORD") {

system ("/var/www/html/php/post_thread.pl \"$player\" \"$password\" \"edit\" \"$article_id\" \"$body\" >/dev/null &", $retval);

}

function create_thread($title,$message,$forum_id='76', $player="USERNAME", $password="PASSWORD") {
...
$result = system ("/var/www/html/check_bgg_user.pl \"$username\"", $retval);
...
function geekmail_form($to="",$subject="",$message="") {
  ...
  $output .= "<tr><td></td><td><input type='checkbox' name='remember' />Remeber BGG password. <a href='javascript:bgg_pwd_note()'>Read First</a></td></tr>\n";
