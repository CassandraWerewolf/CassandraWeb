...
if ( isset($_POST['submit']) || isset($_POST['cancel'])) {
  ...
  if ( isset($_POST['target_id']) ) {
	...
	if ( $_POST['target_id'] != "" ) {
	  if ( $_POST['target_id'] != '0' ) {
        ...
        $result=mysqli_query($mysql, $sql);
        $target = mysqli_result($result,0,0);
        $message .= $target." ";
	  } else {
  ...
  if ( isset($_POST['submit_move']) || isset($_POST['cancel_move'])) {
    ...
    if (isset($_POST['submit_move'])) {
	  ...
	  $result_imm = mysqli_query($mysql, $sql_imm);
	  $processing = 'none';
	  if (mysqli_num_rows($result_imm) > 0) {
	    $processing = mysqli_result($result_imm,0,0);
	  }
      if ($modchat != "") {
        $sql = sprintf("select name from Exits where id=%s",quote_smart($_POST['exit']));
        $result = mysqli_query($mysql, $sql);
        $exit_name = mysqli_result($result,0,0);
    ...
    if ($modchat != "") {
      ...
      $result = mysqli_query($mysql, $sql);
      $item_name = mysqli_result($result,0,0);
    }
    ...
    $inv_result = mysqli_query($inv_sql);
    while ($item = mysqli_fetch_array($inv_result)) {
      ...
      $result_order = mysqli_query($mysql, $sql_order);    
      $item_name = $item['name'];  
      $item_id = $item['id'];
