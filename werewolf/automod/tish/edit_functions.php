...
function create_info_table($template_id,$edit=false) {
  ...
  $output .= "</td></tr>\n";
  $output .= "<tr><td><b>Role Reveal Type</b></td><td>";
  if ( $edit ) {
...
function create_role_table($template_id,$edit=false,$add=0) {
  ...
  if ( $edit ) {
    if  ( $add != 0 ) {
	  for ( $i=1; $i<=$add; $i++ ) {
		...
		$require['0'] = "No";
		$require['1'] = "Yes";
		$output .= create_dropdown('require_role_'.$count,'0',$require);
	    $output .= "</td>";
	    $output .= "</tr>\n";
	  }
	}
    $output .= "<tr><td colspan='14'> ";
  ...
  if ( $edit ) {
	...
	$output .= "<b>Action Description:</b> Currently Automod can only handel Kill, View and Guard orders.  If you would like something added please let Melsana know.<br />";
	$output .= "<b>Group Name:</b>  If players are given the same group name they will be able to see eachother's orders.  They will also be put in a Chat room withat name.<br />";
	...
	$output .= "<b>View Result:</b> If the player gets a view this must beset to 'Positive Hit on:'.  Then you must choose which roles will be considered positive hits.  Then you must choose what sort of result the viewer gets.  If the role viewed is postive then the result will be 'X is a Y' where X is the player targeted and Y is the 'View as:' field.  If the role viewed is a negative hit then the result will be 'X is not a Y'.  <br />";
	...
	$output .= "<b>Attribute:</b> This lets you specify if the role is Tough, or Brutal (neither should be hidden), or Tinker or White Hat (could be hidden).  Tough will have to be killed twice, Brutal will get to choose a player to take down when killed (The order command will be created automatically.)  Tinker will make it so that all views are reversed,  If it would have been positive it will now be negative if it would have been negative it will now be positive.  White Hat means the playes 'Reveal as' information will be revealed on death regardles.<br />";
	...
	$output .= "<b>Require Role:</b>  This will only be used if the number of roles is more than the number of players needed.  This allows you to make games where some roles may or may not be in the game.  A Yes will make sure that role is in the game, like the wolves and seer.<br />";
...
function update_info($data,$template_id) {
  ...
  $sql = sprintf("update AM_template set description=%s, num_players=%s, role_reveal=%s, random_n0=%s, priest_type=%s, random_tinker=%s, random_whitehat=%s where id=%s",quote_smart($data['description']),quote_smart($data['num_players']),quote_smart($data['role_reveal']),quote_smart($data['random_n0']),quote_smart($data['priest_type']),quote_smart($random_tinker),quote_smart($random_whitehat),quote_smart($template_id));
  $result=mysql_query($sql);
...
function update_roles($count,$data,$template_id) {
  for ( $i=1; $i<=$count; $i++ ) {
    ...
    if ( $data['id_'.$i] == 'new' ) {
	  $sql = sprintf("insert into AM_roles (id, template_id, role_id, side, game_action, action_desc, group_name, n0_knows, n0_view, view_result, reveal_as, attribute, a_hidden, parity, require_role) VALUES ( null, %s, %s, %s, %s, $action_desc, $group_name, '$n0_knows', %s, $view_result, %s, %s, %s, %s, %s)",quote_smart($template_id),quote_smart($role_id),quote_smart($side),quote_smart($game_action),quote_smart($n0_view),quote_smart($reveal_as),quote_smart($attribute),quote_smart($a_hidden),quote_smart($parity),quote_smart($require_role));
	  $result = mysql_query($sql);
	} else {
	  $id = $data['id_'.$i];
	  $sql = sprintf("update AM_roles set role_id=%s, side=%s, game_action=%s, action_desc=$action_desc, group_name=$group_name, n0_knows='$n0_knows', n0_view=%s, view_result=$view_result, reveal_as='$reveal_as', attribute=%s, a_hidden=%s, parity=%s, require_role=%s where id=%s",quote_smart($role_id),quote_smart($side),quote_smart($game_action),quote_smart($n0_view),quote_smart($attribute),quote_smart($a_hidden),quote_smart($parity),quote_smart($require_role),quote_smart($id));
	  $result = mysql_query($sql);
	}
  }
  return;
