...
while ( $roles = mysql_fetch_array($result) ) {
  $sql2 = sprintf("insert into AM_roles (id, template_id, role_id, side, game_action, action_desc, group_name, n0_knows, n0_view, view_result, reveal_as, attribute, a_hidden, parity, require_role) VALUES ( null, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )",quote_smart($new_template_id),quote_smart($roles['role_id']),quote_smart($roles['side']),quote_smart($roles['game_action']),quote_smart($roles['action_desc']),quote_smart($roles['group_name']),quote_smart($roles['n0_knows']),quote_smart($roles['n0_view']),quote_smart($roles['view_result']),quote_smart($roles['reveal_as']),quote_smart($roles['attribute']),quote_smart($roles['a_hidden']),quote_smart($roles['parity']),quote_smart($roles['require_role']));
