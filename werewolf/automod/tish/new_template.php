...
if ( isset($_POST['submit'])) {
  ...
  $sql = sprintf("insert into AM_template (id, owner_id, name, description, num_players, role_reveal, random_n0, priest_type, random_tinker, random_whitehat, mode ) VALUES ( null, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'Edit')",quote_smart($uid),quote_smart($_POST['name']),quote_smart($_POST['description']),quote_smart($_POST['num_players']),quote_smart($_POST['role_reveal']),quote_smart($_POST['random_n0']),quote_smart($_POST['priest_type']),quote_smart($random_tinker),quote_smart($random_whitehat));
